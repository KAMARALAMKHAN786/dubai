...
const getBadgeForPlan = (plan: string | null) => {
  switch (plan?.toLowerCase()) {
    case 'team':
      return (
        <div className="absolute top-2 right-2 bg-gradient-to-r from-yellow-400 to-yellow-600 px-3 py-1 rounded-full flex items-center space-x-1 shadow-lg z-10">
...
const Card: React.FC<CardProps> = ({ mainImage, subImages, title, price, description, phone, payment_plan }) => {
      ...
      <div className="w-full h-full max-w-xs mx-auto rounded-lg shadow-lg border border-gray-200 p-4 bg-[#00000091] flex flex-col relative">
        ...
        <div className="flex flex-col flex-grow">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-bold text-white leading-tight text-balance">{title}</h2>
            {payment_plan && (
              <div
          ...
          <p className="text-sm font-semibold text-white mb-2">₹{price}</p>
          <p className="text-sm text-white leading-relaxed flex-grow text-pretty">
